...
use crate::{
	...
	service::{new_partial, AllychainRuntimeExecutor},
};
use codec::Encode;
...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		load_spec(id, self.allychain_id.unwrap_or(2000).into())
	}

	fn native_runtime_version(_: &Box<dyn ChainSpec>) -> &'static RuntimeVersion {
		&rialto_allychain_runtime::VERSION
	}
}
	...
	match &cli.subcommand {
		...
		Some(Subcommand::Benchmark(cmd)) =>
			if cfg!(feature = "runtime-benchmarks") {
				...
				runner.sync_run(|config| cmd.run::<Block, AllychainRuntimeExecutor>(config))
			} else {
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				crate::service::start_node(config, axia_config, id)
					.await
					.map(|r| r.0)
					.map_err(Into::into)
			})
