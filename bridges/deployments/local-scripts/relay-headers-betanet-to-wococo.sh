...
RUST_LOG=rpc=trace,bridge=trace ./target/debug/substrate-relay init-bridge BetaNetToWococo \
...
RUST_LOG=rpc=trace,bridge=trace ./target/debug/substrate-relay relay-headers BetaNetToWococo \
	--source-host 127.0.0.1 \
	--source-port 9955 \
	--target-host 127.0.0.1 \
	--target-port 9944 \
