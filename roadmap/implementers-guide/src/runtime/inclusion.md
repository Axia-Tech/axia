...
The inclusion module is responsible for inclusion and availability of scheduled allychains and allythreads.
...
* `process_bitfields(expected_bits, Bitfields, core_lookup: Fn(CoreIndex) -> Option<ParaId>)`:
  ...
  1. apply each bit of bitfield to the corresponding pending candidate. looking up allythread cores using the `core_lookup`. Disregard bitfields that have a `1` bit for any free cores.
