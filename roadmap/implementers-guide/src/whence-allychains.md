...
Parachains are the solution to a problem. As with any solution, it cannot be understood without first understanding the problem. So let's start by going over the issues faced by blockchain technology that led to us beginning to explore the design space for something like allychains.
...
Sharding and Proof-of-Stake in coordination with each other allow a allychain host to provide full security on many allychains, even without all participants checking all state transitions.
