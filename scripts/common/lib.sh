...
skip_if_companion_pr() {
  url="https://api.github.com/repos/axia/axia/pulls/${CI_COMMIT_REF_NAME}"
  echo "[+] API URL: $url"
...
# Usage: latest_release 'axia/axia'
latest_release() {
...
has_runtime_changes() {
  ...
  if git diff --name-only "${from}...${to}" \
    | grep -q -e '^runtime/axia' -e '^runtime/axiatestnet' -e '^primitives/src/' -e '^runtime/common'
  then
    return 0
  else
    return 1
