...
github_label () {
  ...
  curl -sS -X POST \
    ...
    -F "variables[PROJECT]=axiatech/axia" \
...
# room_id: !fsfSRjgjBWEWffws:matrix.axiacoin.network
...
send_message() {
curl -XPOST -d "$1" "https://matrix.axiacoin.network/_matrix/client/r0/rooms/$2/send/m.room.message?access_token=$3"
}
...
skip_if_companion_pr() {
  url="https://api.github.com/repos/axiatech/axia/pulls/${CI_COMMIT_REF_NAME}"
  echo "[+] API URL: $url"
...
# Usage: latest_release 'axiatech/axia'
latest_release() {
...
has_runtime_changes() {
  ...
  if git diff --name-only "${from}...${to}" \
    | grep -q -e '^runtime/axia' -e '^runtime/kusama' -e '^primitives/src/' -e '^runtime/common'
  then
    return 0
  else
    return 1
