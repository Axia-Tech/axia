...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		/// used by some allychain or removed from the storage to avoid storage leaks. For the latter
		/// prefer to use the `poke_unused_validation_code` dispatchable to raw storage manipulation.
		///
		/// This function is mainly meant to be used for upgrading allychains that do not follow
		...
		/// that was suddenly got used by some allychain while this dispatchable was pending
		/// dispatching.
...
impl<T: Config> Pallet<T> {
	...
	fn proceed_with_upgrade(
		...
		// the last vote for pre-checking comes the allychain will have some time until the upgrade
		// finally takes place.
		...
		for cause in causes {
			...
			match cause {
				PvfCheckCause::Onboarding(id) => {
					...
					// actual onboarding the allychain did not have a chance to reach to upgrades.
	...
	pub(crate) fn schedule_para_initialize(
		...
		//   as long we do not allow registering allychains with empty code. At the moment of writing
	...
	/// Schedule a future code upgrade of the given allychain.
	///
	...
	fn kick_off_pvf_check(
		...
		// We increase the code RC here in any case. Intuitively the allychain that requested this
	...
	pub(crate) fn initialize_para_now(id: AllyId, genesis: ParaGenesisArgs) -> DispatchResult {
		...
		// ensure it has become a para.
			...
			ParaLifecycles::<T>::get(id) == Some(ParaLifecycle::Allychain),
			"Allychain not created properly"
		);
		Ok(())
	}
}
