...
pub struct HostConfiguration<BlockNumber> {
	...
	///
	/// This number is used to prevent allychains from spamming the relay chain with validation code
	/// upgrades. The only thing it controls is the number of blocks the `UpgradeRestrictionSignal`
	/// is set for the allychain in question.
	///
	...
	///
	/// The upgrade for a allychain takes place when the first candidate which has relay-parent >=
	...
	/// upgrade procedure is time-based and allychains that do not look at the go-ahead signal
	/// should still work.
	...
	///
	/// That can disrupt allychain inclusion. Specifically, it will make the blocks that were
	...
	BlockNumber: Zero + PartialOrd + sp_std::fmt::Debug + Clone + From<u32>,
{
	...
	pub fn check_consistency(&self) -> Result<(), InconsistentError<BlockNumber>> {
		...
		if self.hrmp_max_allychain_outbound_channels > crate::hrmp::HRMP_MAX_OUTBOUND_CHANNELS_BOUND
		{
			return Err(MaxHrmpOutboundChannelsExceeded)
		}

		if self.hrmp_max_allychain_inbound_channels > crate::hrmp::HRMP_MAX_INBOUND_CHANNELS_BOUND {
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_hrmp_max_allychain_inbound_channels(
			...
			ensure_root(origin)?;
			Self::schedule_config_update(|config| {
				config.hrmp_max_allychain_inbound_channels = new;
			})
		}
		...
		pub fn set_hrmp_max_allychain_outbound_channels(
			...
			ensure_root(origin)?;
			Self::schedule_config_update(|config| {
				config.hrmp_max_allychain_outbound_channels = new;
			})
		}
		...
		/// Sets the minimum delay between announcing the upgrade block for a allychain until the
...
impl<T: Config> Pallet<T> {
	...
	/// only when enabling allychains runtime pallets for the first time on a chain which has
